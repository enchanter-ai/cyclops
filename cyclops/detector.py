...
class Detector:
    ...
    def feed(self, server: Server, tool: Tool, args: dict[str, Any], result: str) -> ToolCall:
        ...
        if call.is_egress and self.graph.find_toxic_path():
            self.metrics.flag()
        return call
    ...
    def leak_bytes(self) -> int:
        ...
        if not path:
            return 0
        secret = next(c for c in path if c.taint is Taint.SENSITIVE)
        return leaked_bytes(secret.result, path[-1].args)

    def blocks(self, call: ToolCall) -> bool:
        if self.mode is Mode.PREVENT and call.is_egress and self.graph.find_toxic_path():
            self.metrics.block()
            return True
        return False
